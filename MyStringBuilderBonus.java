...
//This program is a MyStringBuilder2 class and it is a linked list implementation of
//the StringBuilder class, which is normally implemented with an ArrayList
//This program also contains a bonus method, lastIndexOf()
...
public class MyStringBuilder2
{
	...
	{
		//Initialize everything to null or zero for empty MyStringBuilder2
		firstC = null;
		lastC = null;
		length = 0;
	}
	...
	{
		//Normal case, MyStringBuilder2 is not empty
		if (length > 0)
		{
			//If passed argument is an empty MyStringBuilder2, return current MyStringBuilder2
			if (b.length() == 0)
			{
				return this;
			}
			//Otherwise, traverse through passed MyStringBuilder2, adding one char at a time using chatAt() method
			else
			{
		...
		//Special case, length of current MyStringBuilder2 is zero
		else
		{
			if (b.length() == 0)
			{
	...
	public MyStringBuilder2 append(String s)
	{
		...
		if (length > 0)
		{
			//If passed argument is an empty String, return current MyStringBuilder2
			if (s.length() == 0)
			{
				return this;
			}
	...
	{
		//Normal case, MyStringBuilder2 length is greater than zero
		if (length > 0)
		{
			if (c.length == 0)
			{
			...
			{
				//Simply traverse through the char array and add one character at a time to the end of the MyStringBuilder2
		...
		//Special case where length of current MyStringBuilder2 is equal to zero
		else
		{
			if (c.length == 0)
			{
	...
	public MyStringBuilder2 append(char c)
	{
		//Special case where the length of the current MyStringBuilder2 is zero
		if (length == 0)
		{
		...
		//Normal case, just add a new node to the end of the MyStringBuilder2
		else
		{
	...
	public char charAt(int index)
	{
		...
		{
			//traverse through the MyStringBuilder2 until the index is reached, then return the data at that node
	...
	{
		//Special case, invalid start and end parameters are given, so current MyStringBuilder2 is returned
		...
		{
			//create two temp nodes to traverse through the MyStringBuilder2 with
			CNode currNode1 = firstC;
			CNode currNode2 = firstC;

			//traverse through the MyStringBuilder2 to start index, changing both of the temp nodes each time
			for (int i = 0; i < start - 1; i++)
			{
			...
			//traverse through the MyStringBuilder2 to end index, only changing the second temp node this time
			...
			//increment length and return MyStringBuilder2
			length = length - (end - start);
			return this;
		}
	...
	{
		//Special case, invalid index is passed, so MyStringBuilder2 is returned
		...
		else
		{
			...
			else
			{
				...
				//Special case, index is at end of MyStringBuilder2
				if (index == length - 1)
					lastC = currNode;

				//Decrement the length and return MyStringBuilder2
				length -= 1;
				return this;
			}
		}
	...
	public int indexOf(String str)
	{
		...
		//While loop that only ends when String is found or end of MyStringBuilder2 is reached
		while (true)
		{
			//If index has reached end of the MyStringBuilder2
			if (index == length - 1)
				break;
			//If the char in MyStringBuilder2 at currNode is equal to first char of passed String
	...
	{
		//if the offset is the length of the MyStringBuilder2, the passed String simply needs appended
		if (offset == length)
		{
			this.append(str);
			return this;
	...
	public MyStringBuilder2 insert(int offset, char c)
	{
		...
		//In all other cases (invalid indices), simply return the current MyStringBuilder2
		else
		{
			return this;
		}
	...
	public MyStringBuilder2 insert(int offset, char [] c)
	{
		...
		//Case in which an invalid index is passed for the offset, the current MyStringBuilder2 is returned
		else if (offset < 0 || offset > length)
			return this;
	...
	{
		//Special case in which the passed parameters are invalid, simply return the current MyStringBuilder2
		...
		//respectively in order to replace a String within the MyStringBuilder2
	...
	{
		//Special case in which the start is 0, no pre-traversing of the MyStringBuilder2 is necessary
		if (start == 0)
		{
			CNode currNode = firstC;
			MyStringBuilder2 newBuilder = new MyStringBuilder2();
			for (int i = 0; i < end; i++)
			{
	...
	{
		//First converts the MyStringBuilder2 to a char array and then converts that to a String and returns it
	...
	//This method is implemented in the standard StringBuilder class in the Java 8 API
	//It takes two arguments, a sting and an index, and finds the last instance of that string
	//in the MyStringBuilder, starting from the given index. It returns -1 if no instance is found.
	//This is a bonus method.
